...
def download_roboflow_dataset(
    ...
    if not api_key:
        raise ValueError("ROBOFLOW_API_KEY not found in environment variables. Please set it in your .env file.")
...
def _process_split(files: List[Path], target_dir: Path) -> None:
    ...
    for img_path in files:
        ...
        if label_path.exists():
            ...
            with open(target_dir / "labels" / label_path.name, 'w') as f:
                f.write("\n".join(new_lines))
...
def print_metrics(results):
    """
    Parses the results.results_dict and prints a clean text table.
    """
    ...
    # Map the ugly keys to pretty names
    key_map = {
    ...
    for key, display_name in key_map.items():
        # Use .get() to safely retrieve the value in case a key is missing
        val = metrics.get(key)
